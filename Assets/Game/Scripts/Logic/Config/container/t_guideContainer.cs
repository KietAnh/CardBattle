...
namespace Data.Containers
{
	public class t_guideContainer : BaseContainer
	{
				...
				try
				{
					...
					while (data.Length > offset)
					{
						...
						if(!map.ContainsKey(bean.t_id))
							...
							Logging.Err("Exist duplicate Key: " + bean.t_id + " t_guideBean");
					}
				}
				catch (Exception ex)
				{
					Logging.Err("import data error: t_guideBean >>" + ex.ToString());
				}
			}
			else
			{
				Logging.Err("can not find conf data: t_guideBean.bytes");
			}
		}
		
		private byte[] getClientData()
		{
            byte[] data = ConfigManager.Singleton.GetData("t_guideBean");
		...
		{
			byte[] data = File.ReadAllBytes(System.Environment.CurrentDirectory + "/bean/t_guideBean.bytes");
			return data;
		}
	}
}
